...
namespace ExcelHelper
{
    ...
    {
        private static int lastCellCout;

        public static int LastCellCount
        {
            get { return lastCellCout; }
            set { lastCellCout = value; }
        }
        ...
        /// <returns></returns>
        public MemoryStream ExportDT(String strFileName, DataTable dtSource, string strHeaderText, Dictionary<string, string> dir, int sheetnum)
        {
            //创建工作簿和sheet
        ...
        public void ExportDTI(DataTable dtSource, string strHeaderText, FileStream fs, MemoryStream readfs, Dictionary<string, string> dir, int sheetnum,bool sheetBookTitle)
        {
        ...
        public void ExportDTtoExcel(DataTable dtSource, string strHeaderText, string strFileName, Dictionary<string, string> dir, bool isNew, int sheetRow ,bool sheetBookTitle)
        {
            ...
            else
            {
                ...
                for (int i = currentSheetCount; i < currentSheetCount + sheetCount; i++)
                {
                    ...
                    using (FileStream writefs = new FileStream(strFileName, FileMode.Create, FileAccess.Write))
                    {
                        ...
                        ExportDTI(pageDataTable, strHeaderText, writefs, readfsm, dir, i , sheetBookTitle);
                    }
                    readfsm.Close();
                }
            }
        ...
        /// <returns></returns>
        public DataTable ImportDt(ISheet sheet, int HeaderRowIndex, Dictionary<string, string> dir)
        {
            ...
            try
            {
                ...
                if (HeaderRowIndex < 0)
                {
                    for (int i = 0; i < sheet.LastRowNum - 1; i++)
                    {
                        ...
                        lastCellCout = cellCount;
                        cellCount = headerRow.LastCellNum - 1;
                        if (lastCellCout > cellCount)
                        {
                            cellCount = lastCellCout;
                        }
                    }
        ...
        /// <param name="dir">excel列名和DataTable列名的对应字典</param>
        /// <returns></returns>
        public DataTable ImportExceltoDt(string strFileName, Dictionary<string, string> dir, string SheetName, int HeaderRowIndex = 1)
        {
            DataTable table = new DataTable();
        ...
        /// <param name="dir">excel列名和DataTable列名的对应字典</param>
        /// <returns></returns>
        public DataTable ImportExceltoDt(string strFileName, Dictionary<string, string> dir, int HeaderRowIndex = 1, int SheetIndex = 0)
        {
            DataTable table = new DataTable();
            using (FileStream file = new FileStream(strFileName, FileMode.Open, FileAccess.Read))
            {
        ...
        /// <returns></returns>
        public int GetSheetNumber(string outputFile)
        {
            int number = 0;
